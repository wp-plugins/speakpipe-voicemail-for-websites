=== Plugin Name ===
Contributors: SpeakPipe
Donate link: http://www.speakpipe.com/
Tags: voicemail, podcast, feedback, customers, contact, testimonials, comments, messages, communication, voice, widget, sidebar, audio message, voice call, contact widget
Requires at least: 3.0.1
Tested up to: 4.0
Stable tag: trunk
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

Allows your customers, blog readers, podcast listeners and fans to send you voice messages right from a browser without any phone calls.

== Description ==

With SpeakPipe you can receive voice messages from your audience directly on your website. Installing SpeakPipe on a website is easy and takes less than a minute. Visitors don't need a SpeakPipe account to send you voicemail. A message can be sent with just a few clicks without typing anything.

This plugin allows site owners to easily integrate their SpeakPipe code into their Wordpress site without having to change any source code or theme files.
Please note that all your voice messages and data are stored on speakpipe.com, for more information please see our Privacy Policy at http://www.speakpipe.com/privacy

Feel free to contact us with any questions at http://www.speakpipe.com/contact or via email support@speakpipe.com

== Installation ==

1. Sign up on SpeakPipe at http://www.speakpipe.com/
2. Install and activate the SpeakPipe plugin
3. Enter your SpeakPipe account ID, you can find it in settings at https://www.speakpipe.com/account/settings
4. Configure your widget: set sidebar button position, color, text in widget settings at https://www.speakpipe.com/account/settings/widget
5. Done! Try to send a voice message using the widget from your website
6. Don't forget to tell your users about your new tool.

== Frequently Asked Questions ==

= Why use SpeakPipe? =
It's a quick and easy way to connect with your audience, and receive questions and testimonials from your customers. With SpeakPipe you can give your customers an easy way to contact you. 

= Who can send me a voice message? =
Anyone who visits your website can send you a voice message. But if you put SpeakPipe on the protected area of your website, only those who have access will be able to send you a message.
Users don't have to sign up on SpeakPipe in order to send you a voice message.

= Can I reply to the message? =
Yes, you can reply to the message via your SpeakPipe inbox.

= How do I know that I received a voice message? =
When a user sends you a message, you will receive a notification via email.

= Is it free? =
Yes, we provide a free account. You can upgrade at any time to a premium account.

= I can't get this thing to work. =
Feel free to contact us at http://www.speakpipe.com/contact or via email support@speakpipe.com and we'll try our best to resolve any issues.

== Changelog ==

= 0.1 =
* Created the plugin.
